...
const mainNav = document.querySelectorAll(".navigationd__right-item");
const underLine = document.querySelector(".under-line");

if (mainNav) {
  mainNav.forEach((item) => {
    item.addEventListener("mouseenter", function (e) {
      const pointZero = mainNav[0].getBoundingClientRect().left;
      const coords = item.getBoundingClientRect().left - pointZero;

      underLine.style.width = `${item.getBoundingClientRect().width}px`;
      underLine.style.transform = `translateX(${coords}px)`;
    });
  });
  mainNav.forEach((item) => {
